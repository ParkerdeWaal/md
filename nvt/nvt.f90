program nvt
implicit none
real*8 :: dt, H, Ke, Kes, px, py, vx, vy, fx, fy, fxold, fyold, potential
...
do n=1,nstep
	call boxmul(dx1,dy1)
	call boxmul(dx2,dy2)
	px = px + s1*vx*dt + s2*dt**2d0*fx + sqrt(sigma1)*dx1
	py = py + s1*vy*dt + s2*dt**2d0*fy + sqrt(sigma1)*dy1
	fxold = fx
	fyold = fy
	call calculate_Forces(px,py,fx,fy)
	vx = s0*vx + (s1-s2)*dt*fxold + s2*dt*fx + sqrt(sigma2)*(crv*dx1+sqrt(1d0-crv**2d0)*dx2)
	vy = s0*vy + (s1-s2)*dt*fyold + s2*dt*fy + sqrt(sigma2)*(crv*dy1+sqrt(1d0-crv**2d0)*dy2)
    ...
    if(MOD(n, 500).eq.0)then
		...
		H = 0.5d0 * (vx**2d0+vy**2d0) + potential
